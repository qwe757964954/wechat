    ...
    execute: function () {
      ...
      _export("OpInfo", OpInfo = (_dec = ccclass('OpInfo'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Node), _dec6 = property(Node), _dec7 = property(Node), _dec8 = property(Layout), _dec9 = property(Layout), _dec10 = property(Layout), _dec11 = property({
        ...
        updateOpration(resp) {
          ...
          var result = this.findOpTypesWithSameNodeAndCode(this.opTypeArr);

          for (var i = 0; i < result.length; i++) {
            var cardArr = (_crd && OperationMgr === void 0 ? (_reportPossibleCrUseOfOperationMgr({
              error: Error()
        ...
        findOpTypesWithSameNodeAndCode(opTypes) {
          ...
          var optypedEntries = Array.from(opTypesMap.entries()).filter(_ref => {
            var [key, opTypes] = _ref;
            return opTypes.length > 1;
          }); // 构建一个新的 Map 包含筛选结果

          var opMap = new Map(optypedEntries); // 从映射中提取所有具有相同 node 和 code 的 OpType 数组

          var result = [];

          for (var [_, group] of opTypesMap) {
            if (group.length > 1) {
              result.push(group);
            }
          }
...
//# sourceMappingURL=2624da61755ecd410bbf969e8f0f8eb04e28360a.js.map