    ...
    execute: function () {
      ...
      _export("ButtonTouchLong", ButtonTouchLong = (_dec = ccclass("ButtonTouchLong"), _dec2 = menu('gui/button/ButtonTouchLong'), _dec3 = property({
        ...
        onTouchtStart(event) {
          ...
          if (uiTransform) {
            //计算滑动的矩形区域
            var width = Math.abs(uiTransform.width) * Math.abs(this.node.scale.x);
            var height = Math.abs(uiTransform.height) * Math.abs(this.node.scale.y);
            var worldPos = new Vec3();
            this.node.getWorldPosition(worldPos); // 矩形左下点的坐标。

            var nodeLeftBottomPos = {
              x: worldPos.x,
              y: worldPos.y
        ...
        onTouchEnd(event) {
          ...
          var oldEvent = this._currowEvent;
          var isTouchLong = this._isTouchLong;
          this._currowEvent = event;
          this.checkRect();
          this._currowEvent = null;
          this._isTouchLong = false;
        ...
        update(dt) {
          if (this._currowEvent && this.interactable == true && this._isCanCallback == true) {
            ...
            if (this._currowPassTime >= this.touchlongtime) {
              var status = ButtonTouchLongStatus.RUN;
        ...
        checkRect() {
          if (this._currowEvent) {
            var touchWorldPos = this._currowEvent.touch.getUILocation();
        ...
        onLongClickCallback(event, status) {
...
//# sourceMappingURL=9b3e7e63db293b0527816c95569879d65bf16c48.js.map