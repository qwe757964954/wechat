  ...
  return {
    ...
    execute: function () {
      ...
      _export("giftDiscountCtr", giftDiscountCtr = (_dec = ccclass('giftDiscountCtr'), _dec2 = property({
        ...
        onLoad() {
          //获取上一个付费场景
          var param = this.getComponent(_crd && DelegateComponent === void 0 ? (_reportPossibleCrUseOfDelegateComponent({
            error: Error()
        ...
        updateView() {
          ...
          var maxDataItem = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            error: Error()
          }), Utils) : Utils).table_verify(this._holidayConf['giftpack'], true).length;
          this._selectedIndex = -1;

          for (var i = 0; i < maxDataItem; i++) {
            var data = this.getItemData(i);
          ...
          var currowNum = this.itemPanel.children.length;

          if (currowNum < maxDataItem) {
            for (var _i = currowNum; _i < maxDataItem; _i++) {
              var node = instantiate(this.nodeBoxGift);
              this.itemPanel.addChild(node);
            }
          } else if (currowNum > maxDataItem) {
            for (var _i2 = currowNum - 1; _i2 >= maxDataItem; _i2--) {
              var _node = this.itemPanel.children[_i2];
          ...
          for (var _i3 = 0; _i3 < this.itemPanel.children.length; _i3++) {
            var nodeItem = this.itemPanel.children[_i3];
            count = count + 1;
            this.updateItemBox(nodeItem, _i3);
          }

          var self = this;
          var layout = self.itemPanel.getComponent(Layout);
        ...
        updateButton() {
          ...
          if (this._holidayConf['giftpack']) {
            var item = this._holidayConf['giftpack'][this._selectedIndex];
        ...
        getItemData(idx) {
          ...
          var item = this._holidayConf['giftpack'][idx];
          ...
          var maxNum = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            error: Error()
          }), Utils) : Utils).number_valueOf(payData['limitNum'], 0);
          var isLimit = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            error: Error()
        ...
        updateItemBox(node, idx) {
          ...
          var bg = node.getChildByName("bg"); //选中

          var check = bg.getChildByName("check"); //蒙版

          var mask = bg.getChildByName("mask"); //购买

          var buy = bg.getChildByName("buy");
          var imgLabel = node.getChildByName("imgLabel");
          var labelNum = imgLabel.getChildByName("labelNum");
          var data = this.getItemData(idx);
          ...
          this.getPreLoaderRemoteRes(iconUrl, '.jpg', imageAsset => {
            ...
            if (spriteFrame) {
              var spriteComponent = bg.getComponent(Sprite);
              spriteComponent.spriteFrame = spriteFrame;
            }
          });
          var buttonSimple = node.getComponent(_crd && ButtonSimple === void 0 ? (_reportPossibleCrUseOfButtonSimple({
            error: Error()
        ...
        onClickChooseItem(event) {
          ...
          var conf = this._holidayConf['giftpack'][idx];
          var goodsId = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            error: Error()
          ...
          var _currowSelectItemBg = this._currowSelectItemBg; //老的成为未选中

          _currowSelectItemBg.getChildByName("check").active = false;

          var oldSelect = _currowSelectItemBg.getParent().getChildByName("select");
          ...
          var newSelect = this._currowSelectItemBg.getParent().getChildByName("select");
        ...
        _buyItem(idx) {
          ...
          var payID = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            error: Error()
          ...
          var boughtNum = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            error: Error()
...
//# sourceMappingURL=e1e67c60eda73a227f7317dd4fb246a3e50a8599.js.map