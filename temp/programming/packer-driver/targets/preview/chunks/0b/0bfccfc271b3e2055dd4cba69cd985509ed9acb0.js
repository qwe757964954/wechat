    ...
    execute: function () {
      ...
      _export("RedDot", RedDot = class RedDot extends (_crd && BaseCache === void 0 ? (_reportPossibleCrUseOfBaseCache({
        ...
        has_reddot_signin() {
          ...
          var res = false;
          var data = this._redConfData[(_crd && GConstants === void 0 ? (_reportPossibleCrUseOfGConstants({
            error: Error()
        ...
        has_reddot_mail() {
          var data = this.__User.MailInfo.getDataByLocal();
        ...
        has_reddot_userHead(item_id) {// let type = GConstants.RedDotConf.UserHead;
        ...
        need_show_newuserActivity() {
          ...
          var isRegisterToday = this.__User.isRegisterToday(); //上次打开的时间
          ...
          var lastOpenTime = (_crd && LocalStorage === void 0 ? (_reportPossibleCrUseOfLocalStorage({
            error: Error()
          }), LocalStorage) : LocalStorage).get(key, null);
          ...
          if (isRegisterToday) {
            ...
            var isFirstLoginCurrow = this.__User.isFirstLoginCurrow();
        ...
        update_dress_red(item_id, status) {// if (item_id != null) {
        ...
        update_Activity_NormalRed(activityData, status) {
          if (status === void 0) {
            status = false;
          }

          var key = (_crd && GConstants === void 0 ? (_reportPossibleCrUseOfGConstants({
            error: Error()
          }), GConstants) : GConstants).RedDotConf.ActivityNormal;

          if (activityData != null) {
            var config_id = activityData["config_id"]; //红点类型 0 无 1.每天一次 2.每次登陆 3.活动期间

            var red_dot_type = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
              error: Error()
            }), Utils) : Utils).number_valueOf(activityData["red_dot_type"], 0); //开始时间

            var begin_time = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
              error: Error()
            }), Utils) : Utils).number_valueOf(activityData["begin_time"], null); //结束时间

            var end_time = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
              error: Error()
            ...
            var oldStatus = this._redConfLocalData[key][config_id]; //是否转发消息

            var isForward = false; //今天的时间戳

            var currowTime = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
              error: Error()
            }), Utils) : Utils).time();
            console.log('红点类型', red_dot_type, 'status', status, 'oldStatus', oldStatus);
        ...
        has_reddot_activityNormal(config_id) {
          if (config_id === void 0) {
            config_id = null;
          }

          var type = (_crd && GConstants === void 0 ? (_reportPossibleCrUseOfGConstants({
            error: Error()
          }), GConstants) : GConstants).RedDotConf.ActivityNormal;
          ...
          for (var key in this._redConfLocalData[type]) {
            if (Object.prototype.hasOwnProperty.call(this._redConfLocalData[type], key)) {
              var status = this._redConfLocalData[type][key];
        ...
        update_Activity_NoticeRed(activityData, status) {
          if (status === void 0) {
            status = false;
          }
        ...
        has_reddot_activityNotice(config_id) {
          if (config_id === void 0) {
            config_id = null;
          }
        ...
        check_reddot_by_name(name, key) {
          var realKey = String(key) + String(name);
        ...
        update_reddot_by_name(name, key) {
          ...
          var res = this.__User.setTodayLocalRecord(realKey, true);

          if (res) {
            var param = this.hasRedDot(key, name); //通知:红点更新
        ...
        has_reddot_firstPay() {
          var flag = this.check_reddot_by_name('gift_firstpay_red_dot', (_crd && GConstants === void 0 ? (_reportPossibleCrUseOfGConstants({
            error: Error()
          }), GConstants) : GConstants).RedDotConf.FirstPay);
          return flag != true;
        }
        ...
        has_reddot_holiday() {
          var flag = this.check_reddot_by_name('gift_holiday_red_dot', (_crd && GConstants === void 0 ? (_reportPossibleCrUseOfGConstants({
            error: Error()
          }), GConstants) : GConstants).RedDotConf.Holiday);
          return flag != true;
        }
...
//# sourceMappingURL=0bfccfc271b3e2055dd4cba69cd985509ed9acb0.js.map