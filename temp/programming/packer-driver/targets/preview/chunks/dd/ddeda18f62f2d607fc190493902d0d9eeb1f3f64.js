    ...
    execute: function () {
      ...
      _export("MyPlayerCard", MyPlayerCard = (_dec = ccclass('MyPlayerCard'), _dec2 = property({
        ...
        cardAddTouchEvent(handNode) {
          ...
          handNode.off(Node.EventType.MOUSE_DOWN);
          handNode.off(Node.EventType.MOUSE_DOWN, event => {
            // console.log(event);
            this._touchNode = event.target;
          }, this);
          handNode.off(Node.EventType.TOUCH_START, event => {
            // console.log(event);
            this._touchNode = event.target;
          }, this);
        ...
        onTouchMove(event) {
          if ((_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            error: Error()
          }), Utils) : Utils).isNull(this._touchNode)) {
            return;
          } // 获取当前触摸点的位置


          var touchPos = event.getLocation(); // 获取之前的触摸点位置

          var prevTouchPos = event.getPreviousLocation(); // 计算Y轴上的移动距离

          var deltaY = touchPos.y - prevTouchPos.y;
          console.log("deltaY_________________", deltaY); // 在Y轴上移动相应的位置

          var newPosition = this.node.position.add(new Vec3(0, deltaY, 0));

          this._touchNode.setPosition(newPosition);
        }
...
//# sourceMappingURL=ddeda18f62f2d607fc190493902d0d9eeb1f3f64.js.map