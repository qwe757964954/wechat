    ...
    execute: function () {
      ...
      _export("Carditem", Carditem = (_dec = ccclass('Carditem'), _dec2 = property({
        ...
        updateCardItemProp(opTypes) {
          ...
          var _loop = function _loop(i) {
            ...
            var _loop2 = function _loop2(idx) {
              ...
              item.on(Node.EventType.TOUCH_END, event => {
                var id_num = event.target.getComponent(_crd && Item === void 0 ? (_reportPossibleCrUseOfItem({
                  error: Error()
                }), Item) : Item).index;

                _this.opClick(opType.code, id_num);
              }, _this);
              rowNum++;
              item_width = cards.length * 53; //= item_width;

              item.getComponent(Layout).updateLayout(); // item_width = item.getComponent(UITransform).width 
          ...
          var constraintNum = rowNum > 2 ? 2 : rowNum;
          cardLayout.updateLayout(); // let transform = ;

          this.node.getComponent(UITransform).height = Math.round(rowNum / 2) * 86; // console.log("transform____________________",item_width,transform.width);

          this.node.getComponent(UITransform).width = constraintNum * (item_width + cardLayout.spacingX); // console.log("transform____________________",transform.width);
...
//# sourceMappingURL=b8402a877042c4cd1546a2f868731afe1e314c8b.js.map