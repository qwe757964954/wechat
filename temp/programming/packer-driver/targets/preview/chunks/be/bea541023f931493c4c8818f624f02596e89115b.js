System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2", "__unresolved_3", "__unresolved_4", "__unresolved_5"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, AppEvent, GConstants, EventManager, Utils, BaseCache, ShareInfo, _crd;
  ...
  function _reportPossibleCrUseOf__InterFace_WXSdk_OnShareAppMessage(extras) {
    _reporterNs.report("__InterFace_WXSdk_OnShareAppMessage", "../platform/weixin/WXSdkDefines", _context.meta, extras);
  }
    ...
    execute: function () {
      ...
      _export("ShareInfo", ShareInfo = class ShareInfo extends (_crd && BaseCache === void 0 ? (_reportPossibleCrUseOfBaseCache({
        ...
        getAwardByNextCount() {
          ...
          var rewards = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            error: Error()
        ...
        isAwardNext() {
          var count = this.getShareCount();
          count = count + 1;
          var totalCount = this.getAwardCount();
        ...
        notifyConfigUpdate(body) {
          if (body) {
            var newVer = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
              error: Error()
            }), Utils) : Utils).number_valueOf(body["ver"], -1);
            var localVer = this.getShareConfVer();
        ...
        shareSuccessTimeAdd(gid) {
          //判断是否是之前点击过的商品
          var _gidIndex = this.goodsgidArray.findIndex(item => {
            return item['gid'] == gid;
          });
        ...
        getAfterSharetime(gid) {
          var currentTime = this.goodsgidArray.find(item => {
            return item['gid'] == gid;
          });
          return currentTime ? {
        ...
        shareTimeAllowed(gid, shareType) {
          var shopShareMsg = this.getShareConfigByType(shareType);
          var shareParams = {
            title: shopShareMsg[0]['title'],
            //转发标题，不传则默认使用当前小游戏的昵称
            imageUrl: shopShareMsg[0]['img']
          };
          var coolDownTime = shopShareMsg[0]['cool_down_type'] ? shopShareMsg[0]['cool_down_time'] * 60 : 0; //当前物品 距离上次分享的时间

          var lastShareMSg = this.getAfterSharetime(gid);
          var allowShareMsg = {
            gid: gid,
            allowShare: true,
            //默认允许分享
...
//# sourceMappingURL=bea541023f931493c4c8818f624f02596e89115b.js.map