System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2", "__unresolved_3", "__unresolved_4", "__unresolved_5", "__unresolved_6", "__unresolved_7", "__unresolved_8", "__unresolved_9", "__unresolved_10"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, Label, Layout, Node, Sprite, SpriteAtlas, _decorator, instantiate, GCache, AppEvent, AppSound, GConstants, GameTxt, Utils, Encrypt, resLoader, EventManager, BaseComponent, _dec, _dec2, _dec3, _dec4, _dec5, _dec6, _dec7, _dec8, _dec9, _dec10, _dec11, _dec12, _dec13, _dec14, _dec15, _dec16, _class, _class2, _descriptor, _descriptor2, _descriptor3, _descriptor4, _descriptor5, _descriptor6, _descriptor7, _descriptor8, _descriptor9, _descriptor10, _descriptor11, _descriptor12, _descriptor13, _descriptor14, _crd, ccclass, property, menu, mailPrefabCtr;
      ...
      _export("mailPrefabCtr", mailPrefabCtr = (_dec = ccclass('mailPrefabCtr'), _dec2 = menu('prefab/activity/mailPrefabCtr'), _dec3 = property({
        ...
        showView() {
          ...
          if (this.curentIndex != null) {
            var oldID = this.currentData["id"];
            var newDataArr = this.getMailDataByID(oldID);
          ...
          for (var i = 0; i < maxlenth; i++) {
            var mailStatus = this.mailData[i].status;
            var data = this.mailData[i];
            var node = instantiate(this.mailItem);
            this.mailLayer.addChild(node);
            node["mailData"] = this.mailData[i];
            node["index"] = i;
            var img_new = node.getChildByName("img_new");
            var img_gift = node.getChildByName("img_gift");

            if (this.curentIndex == null && i == 0) {
              var currentData = this.mailData[0];
              ...
              var _currentData = this.mailData[i]; //界面打开状态

              this.initRightView(_currentData);
              node.getChildByName("img_selected").active = true;
              this.updateCurrentData();
            } //设置标题


            var titleNode = node.getChildByName("label_title");
            titleNode.getComponent(Label).string = data.title; //设置日期

            var timeNode = node.getChildByName("label_time");
            var time = "";

            if (data.time != null) {
              var timeArray = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
                error: Error()
            ...
            var giftSprite = img_gift.getComponent(Sprite);
        ...
        initRightView(dataList) {
          ...
          if (!(_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            ...
            for (var i = 0; i < awardArr.length; i++) {
              var v = awardArr[i];
              var node = instantiate(this.itemNode);
              this.itemLayer.addChild(node);
              var numLabel = node.getComponentInChildren(Label);
              var img_item = node.getChildByName("img_item");
              numLabel.string = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
                error: Error()
              }), Utils) : Utils).string_moneyToShortNumber(v.num, true);
              ...
              var propType = (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
                error: Error()
          ...
          var senderLabel = show_layout.getChildByName("label_sender");
          var timeLabel = show_layout.getChildByName("label_time");
        ...
        onMailClick(event) {
          ...
          for (var child of this.mailLayer.children) {
          ...
          img_new.active = false;
          var img_gift = node.getChildByName("img_gift");
          var currentData = node["mailData"];
          var awardArr = (_crd && Encrypt === void 0 ? (_reportPossibleCrUseOfEncrypt({
            error: Error()
          }), Encrypt) : Encrypt).JsonDecode(currentData.awards); //文字点击即算opened ,奖励要领取才算
          ...
          if (currentData.status == (_crd && GConstants === void 0 ? (_reportPossibleCrUseOfGConstants({
            error: Error()
          }), GConstants) : GConstants).MsgStatus.MSG_STATUS_READ && (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            error: Error()
          }), Utils) : Utils).table_isEmpty(awardArr)) {
        ...
        updateCurrentData() {
          var index = Number(this.curentIndex);
          var len = this.mailLayer.children.length;

          for (var i = 0; i < len; i++) {
            var node = this.mailLayer.children[index];
        ...
        setMailBtn(index) {
          ...
          for (var i = 0; i < this.btnLayer.children.length; i++) {
            var child = this.btnLayer.children[i];
        ...
        onReceiveClick(event) {
          ...
          if (this.currentData && !(_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
            ...
            if (this.currentData.status == (_crd && GConstants === void 0 ? (_reportPossibleCrUseOfGConstants({
              ...
              var img_item = this.currentItem.getChildByName("img_item");
              img_item.getComponent(Sprite).grayscale = true;
              this.setMailBtn();
            } else {
...
//# sourceMappingURL=0f813c092726aa25438718471e678f56594e0645.js.map