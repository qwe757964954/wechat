System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2", "__unresolved_3", "__unresolved_4", "__unresolved_5", "__unresolved_6", "__unresolved_7", "__unresolved_8", "__unresolved_9", "__unresolved_10", "__unresolved_11"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, animation, Animation, AnimationClip, instantiate, Node, Vec3, _decorator, GCache, EventManager, Utils, BaseComponent, OperationMgr, PlayerMgr, OPCode, GameEvent, CardUtil, NodeCardCtr, drawCardDisRatio, MjSize, _dec, _dec2, _class, _class2, _descriptor, _crd, ccclass, property, MyPlayerCard;
  ...
  return {
    setters: [function (_unresolved_) {
      ...
      NodeCardCtr = _unresolved_11.NodeCardCtr;
    }, function (_unresolved_12) {
      drawCardDisRatio = _unresolved_12.drawCardDisRatio;
      MjSize = _unresolved_12.MjSize;
    }],
    execute: function () {
      _crd = true;
      ...
      __checkObsolete__(['animation', 'Animation', 'AnimationClip', 'EventTouch', 'instantiate', 'Node', 'Vec3', '_decorator']);
      ...
      _export("MyPlayerCard", MyPlayerCard = (_dec = ccclass('MyPlayerCard'), _dec2 = property({
        ...
        constructor() {
          ...
          this.clickThreshold = 0.5;
          this.moveThreshold = 20;
          this.startPosition = new Vec3();
          this.movePosition = new Vec3();
        ...
        stackAnim(indexList, tByte, insertIndex, cb) {
          ...
          if (indexList.length === 2) {
            ...
            this.removeHandCard(indexList); //杠牌直接移除三张牌, 刷新位置
        ...
        cardAddTouchEvent(handNode) {
          ...
          handNode.on(Node.EventType.TOUCH_END, event => {
            var currentTime = new Date().getTime() / 1000; // 转换为秒
            // 判断两次点击的时间间隔是否小于阈值
            // if (currentTime - this.lastClickTime < this.clickThreshold) {
            //     // 双击事件逻辑
            //     console.log("Double Clicked!");

            this.playSelectAnim(event.target); // 在这里执行你想要的双击操作
            // }else{
            //     this.playUnselectAnim(event.target);
            // }
        ...
        onTouchStart(event) {
          ...
          this.movePosition = new Vec3(0, 0, 0);
        }

        onTouchMove(event) {
          this._isTouched = true;
          var location = event.getDelta(); // Vec3.add(this.movePosition,this.startPosition,new Vec3(location.x,location.y,0));

          Vec3.add(this.touchNodePos, this.startPosition, new Vec3(location.x, location.y, 0)); // this.movePositionY=  this.startPosition + event.getDelta();
          // if(this.movePositionY > this.moveThreshold){
          //     this.movePositionY = this.moveThreshold;
          // }else if(this.movePositionY < this.moveThreshold){
          //     this.movePositionY = -this.moveThreshold;
          // }

          console.log("movePositionY__________", this.touchNodePos);

          this._touchNode.setPosition(this.touchNodePos);
        }

        onTouchEnd(event) {
          console.log("this.node.onTouchEnd________");
        }

        onTouchCancel(event) {// if(this.movePositionY > 0){
          //     this.playSelectAnim(this._touchNode);
          // }else{
          //     this._touchNode.setPosition(this._touchNode.getPosition().x,-this.movePositionY,this._touchNode.getPosition().z);
          // }
          // this._touchNode = null;
        ...
        playSelectAnim(node) {
          if (this.selectedNode) {
            console.log("this.selectedNode存在_________________");
          ...
          console.log("this.selectedNode不存在_________________");
          ...
          if (this.selectedNode.getComponent(Animation) == null) {
            ...
            animCom.removeAll(Animation.EventType.FINISHED);
            console.log("this.selectedNode存在turnMe_________________");
        ...
        onSendCard(byte) {
          ...
          console.log("onSendCard______", index, byte, (_crd && CardUtil === void 0 ? (_reportPossibleCrUseOfCardUtil({
            error: Error()
          }), CardUtil) : CardUtil).getMajiangValue(byte));
          var req = {
            opCard: byte,
...
//# sourceMappingURL=64c0017fc3c8b70425de14dce5694f880db0c57c.js.map