          ...
          for (const [key, opTypes] of opTypesMap) {
            if (opTypes.length > 1) {
              ...
              if ((_crd && OperationMgr === void 0 ? (_reportPossibleCrUseOfOperationMgr({
                error: Error()
              }), OperationMgr) : OperationMgr).getInstance().isMoreOpration(opTypes[0].code)) {
                opMap.set(key, opTypes);
              }
            }
          }

          console.log("Key_______________________", opMap); // 从映射中提取所有具有相同 node 和 code 的 OpType 数组
...
//# sourceMappingURL=8448ec54f5fdcd0fb73a98ed95c3696ffcad3a66.js.map