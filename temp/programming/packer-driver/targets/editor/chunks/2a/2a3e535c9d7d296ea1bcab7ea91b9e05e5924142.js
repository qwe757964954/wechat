  ...
  return {
    ...
    execute: function () {
      ...
      _export("SuperLayout", SuperLayout = (_dec = ccclass('SuperLayout'), _dec2 = requireComponent(UITransform), _dec3 = menu('gui/scrollview/SuperLayout'), _dec4 = property({
          ...
          if (!this._centerPosition) {
            ...
            if (this.autoCenter) {
              if (this.centerNode) {
                ...
                let worldPos = (_this$centerNode$pare = this.centerNode.parent) == null ? void 0 : (_this$centerNode$pare2 = _this$centerNode$pare._uiProps.uiTransformComp) == null ? void 0 : _this$centerNode$pare2.convertToWorldSpaceAR(this.centerNode.position);
        ...
        total(count, refreshLastItem = false) {
        ...
        scrollToIndex(index, timeInSecond, boundary, reverse = false) {
          ...
          let worldPos = (_this$transform3 = this.transform) == null ? void 0 : _this$transform3.convertToWorldSpaceAR(itemPos);
        ...
        soonFinish() {
          ...
          for (let i = 0; i < this.node.children.length; i++) {
            ...
            const child = this.node.children[i];
            let worldPos = (_this$transform4 = this.transform) == null ? void 0 : _this$transform4.convertToWorldSpaceAR(child.position);
            let localPos = this.view.convertToNodeSpaceAR(worldPos);
            let map = {
              width: false,
              height: false
        ...
        setItemPosition(item, relative, reverse = false, isHeader = false) {
          var pos = new Vec3();
        ...
        refreshItems(offset, refreshLastItem = false) {
          if (offset < 0) {
            for (let i = 0; i < -offset; i++) {
              if (this.headerLoop) {
                this.pushToHeader();
        ...
        createItems(count, refreshLastItem = false) {
          // 有多余的item 需要删除 不处理
          ...
          for (let i = 0; i < total; i++) {
            let child = instantiate(this.cloneNode);
            const transform = child.getComponent(UITransform);
            this.setAndSaveSizeAndScale(transform);
            ...
            child.on(Node.EventType.SIZE_CHANGED, () => {
              this.onChangeChildSize(transform);
            }, this, true);
            child.on(Node.EventType.TRANSFORM_CHANGED, type => {
              this.onChangeChildScale(type, transform);
            }, this, true);
            this.notifyRefreshItem(child);
        ...
        async onChangeChildScale(type, item) {
        ...
        resetStrectchItems() {
          if (!isNaN(this.stretchLock.index)) {
            const index = this.node.children.findIndex(item => item["__index"] == this.stretchLock.index);
        ...
        resetChilds(start = false) {
          ...
          if (start) {
            if (this.vertical) {
              ...
              let x = this.getStartX(this.header);
              (_this$header6 = this.header) == null ? void 0 : _this$header6.node.setPosition(new Vec3(x, this.header.node.position.y));
            } else {
              var _this$header7;

              let y = this.getStartY(this.header);
              (_this$header7 = this.header) == null ? void 0 : _this$header7.node.setPosition(new Vec3(this.header.node.position.x, y));
            }
          }
        ...
        getScaleWidth(trans) {
          if (!trans) return 0;
          const size = trans.node["__runtime_size"];
...
//# sourceMappingURL=2a3e535c9d7d296ea1bcab7ea91b9e05e5924142.js.map