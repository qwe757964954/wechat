  ...
  return {
    ...
    execute: function () {
      ...
      _export("NodeHandCardCtr", NodeHandCardCtr = (_dec = ccclass('NodeHandCardCtr'), _dec2 = property({
        ...
        initView() {
          let num = 14;
          let scale = (_crd && GameViewConfig === void 0 ? (_reportPossibleCrUseOfGameViewConfig({
            error: Error()
          ...
          let thickness = (_crd && GameViewConfig === void 0 ? (_reportPossibleCrUseOfGameViewConfig({
            error: Error()
        ...
        resetView() {
          ...
          for (let dir = 0; dir < this._seatList.length; dir++) {
            ...
            for (let i = 0; i < cards.length; i++) {
              ...
              cards[i].setPosition(pos.x, this._groundToCard, (_crd && GameViewConfig === void 0 ? (_reportPossibleCrUseOfGameViewConfig({
                error: Error()
              }), GameViewConfig) : GameViewConfig).CardConfig.CenterToHandCard);
            }
          }
        ...
        playAnimOfSortByDir(dir, t1, t2) {
          ...
          for (let i = 0; i < cards.length; i++) {
            ...
            let point = new Vec3(pos.x, 0, pos.z + this._thickness - (_crd && GameViewConfig === void 0 ? (_reportPossibleCrUseOfGameViewConfig({
              error: Error()
            }), GameViewConfig) : GameViewConfig).CardConfig.GroundToCard);
            let tweenTargetVec3 = new Vec3(0, 0, 0);
            let tw = new Tween(tweenTargetVec3).by(t1, new Vec3(90, 0, 0), {
        ...
        normalOutCardAny(dir, count) {
          ...
          for (let i = 0; i < num; i++) {
            let pos = cards[i].getPosition(); // 使用动画移动到目标位置
        ...
        playAnimOfShow() {
          ...
          for (let dir = 0; dir < this._seatList.length; dir++) {
            ...
            for (let i = 0; i < cards.length; i++) {
              ...
              let rotaionEuler = new Vec3(cards[i].eulerAngles);
              let pos = new Vec3(cards[i].getPosition());
              let point = new Vec3(pos.x, 0, pos.z - (_crd && GameViewConfig === void 0 ? (_reportPossibleCrUseOfGameViewConfig({
                error: Error()
              }), GameViewConfig) : GameViewConfig).CardConfig.GroundToCard);
              let tweenTargetVec3 = new Vec3(0, 0, 0);
              let tw = new Tween(tweenTargetVec3).by(duration, new Vec3(-90, 0, 0), {
                onUpdate: (target, ratio) => {
                  (_crd && Utils === void 0 ? (_reportPossibleCrUseOfUtils({
                    error: Error()
                  }), Utils) : Utils).rotate3DAroundByPositionRotaion(cards[i], pos, rotaionEuler, point, target, true);
...
//# sourceMappingURL=19a09d725c6c8c0945d2732a59cc7b07ae09bd72.js.map