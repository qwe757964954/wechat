    ...
    execute: function () {
      ...
      _export("MyPlayerCard", MyPlayerCard = (_dec = ccclass('MyPlayerCard'), _dec2 = property({
        ...
        showDrawCard() {
          ...
          let tw = new Tween(tweenTargetVec3).by(duration, new Vec3(0, 0, 0), {
        ...
        cardAddTouchEvent(handNode) {
          ...
          handNode.on(Node.EventType.TOUCH_END, event => {
            this._touchNode.setPosition(this.touchNodePos.x, 0, 0);
        ...
        cancelOpcardTouch(opcard) {
          ...
          for (let i = 0; i < this._mjList.length; i++) {
            let handNode = this._mjList[i];
            let byte = handNode.getComponent(_crd && NodeCardCtr === void 0 ? (_reportPossibleCrUseOfNodeCardCtr({
              error: Error()
            }), NodeCardCtr) : NodeCardCtr).byte; // console.log("cancelOpcardTouch____________",byte,opcard,CardUtil.getMajiangValue(byte),CardUtil.getMajiangValue(opcard));
...
//# sourceMappingURL=1955bc8e053ae110b2d630017bee1abe34e3fcbb.js.map